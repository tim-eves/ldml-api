...
impl Tag {
    ...
    pub fn privateuse<T: AsRef<str>>(p: T) -> Self {
        Tag {
            private: Some(p.as_ref().to_owned()),
            ..Default::default()
        }
    }

    pub fn script<T: AsRef<str>>(mut self, s: T) -> Self {
        self.script = Some(s.as_ref().to_owned());
        self
    }

    pub fn region<T: AsRef<str>>(mut self, r: T) -> Self {
        self.region = Some(r.as_ref().to_owned());
        self
    }

    pub fn private<T: AsRef<str>>(mut self, p: T) -> Self {
        self.private = Some(p.as_ref().to_owned());
        self
    }

    pub fn variants<T, C: AsRef<[T]>>(mut self, c: C) -> Self
    where
        T: AsRef<str> + ToOwned,
    {
    ...
    pub fn extensions<T, C: AsRef<[T]>>(mut self, c: C) -> Self
    where
        T: AsRef<str> + ToOwned,
    {
...
mod tests {
    ...
    fn grandfathered() {
        let gf_cases = [
            ...
            (
                "en-GB-oed",
                Ok(Tag::lang("en").region("GB").add_variant("oxendict")),
            ),
    ...
    fn complex() {
        ...
        let gf_cases = [
            ...
            (
                "en-gan-yue-Latn",
                Ok(Tag::lang("en-gan-yue").script("Latn").clone()),
            ),
        ];
        for (test, result) in &gf_cases {
            assert_eq!(test.parse::<Tag>(), *result);
